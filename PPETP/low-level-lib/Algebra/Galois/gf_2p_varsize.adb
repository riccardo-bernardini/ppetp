...
package body Gf_2p_Varsize is
   ...
   -- Please: read the note on gf_2p_varsize.ads file
   ...
   begin
      ...
      if (X and Overflow_Mask) = 0 then
         return Galois(Shift_Left (Basic_Type(X), 1));
      else
         return Galois(Shift_Left (Basic_Type(X and not Overflow_Mask), 1)) xor Carry_Mask;
      end if;
   ...
   begin
      ...
      while My_Right /= 0 loop
         ...
         My_Right := Galois(Shift_Right(Basic_Type(My_Right), 1));
   ...
   begin
      ...
      begin
         ...
         Tmp := Galois(Shift_Left(Basic_Type(Tmp), Deg(2)-Deg(1)));
         Matrix := ( (X,                  One,                Zero),
   ...
   -- Random_Galois --
   -------------------
   function Random_Galois return Galois is
   begin
      return Galois(Gf_2p_Random.Random(G));
   end  Random_Galois;
